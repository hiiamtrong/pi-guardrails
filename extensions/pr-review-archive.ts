...
const DATABASE_PATH =
  process.env.PI_PR_REVIEW_ARCHIVE_DB ??
...
function repositoryFromGit(cwd: string): string {
  try {
    return normalizeRepository(
      run("git", ["config", "--get", "remote.origin.url"], cwd),
    );
  } catch {
...
function archivePullRequest(
  ...
  const statements = [
    ...
    ...files.map(
      (file) =>
        `INSERT INTO pr_files VALUES (${sql(repository)}, ${sql(number)}, ${sql(file.filename)}, ${sql(file.status)}, ${sql(file.additions)}, ${sql(file.deletions)}, ${sql(file.patch)}, ${sql(file.sha)});`,
    ),
  ];
  let count = 0;
...
async function sync(
  ...
  for (const author of options.authors) {
    for (const pullRequest of ghJson<PullRequest[]>([
      "pr",
      "list",
      "--repo",
      options.repo!,
      ...
      "--json",
      "number,title,state,url,author,baseRefName,headRefName,updatedAt",
    ])) {
  ...
  for (const pullRequest of pullRequests.values())
    comments += archivePullRequest(options.repo!, pullRequest);
  notify(
    ctx,
...
function show(number: number, ctx: ExtensionContext): void {
  ...
  const rows = sqlite(
    `SELECT kind, author_login, body, path, line, diff_hunk, url FROM review_comments WHERE repository=${sql(repository)} AND pr_number=${sql(number)} ORDER BY created_at;`,
    true,
  ) as Array<Record<string, unknown>>;
  if (!rows.length) {
    notify(
      ctx,
      `No archived human comments for ${repository}#${number}. Run /pr-review-archive sync first.`,
      "warning",
    );
    return;
  }
  notify(
    ...
    rows
      .map(
        (row) =>
          `[${row.kind}] @${row.author_login}${row.path ? ` (${row.path}:${row.line ?? "?"})` : ""}\n${row.body}${row.diff_hunk ? `\n\n${row.diff_hunk}` : ""}\n${row.url ?? ""}`,
      )
      .join("\n\n"),
  );
}
...
function search(query: string, ctx: ExtensionContext): void {
  ...
  const where = terms
    .map(
      (term) =>
        `lower(body || ' ' || coalesce(path, '') || ' ' || coalesce(diff_hunk, '')) LIKE ${sql(`%${term}%`)}`,
    )
    .join(" AND ");
  const rows = sqlite(
    `SELECT pr_number, kind, author_login, body, path, line, url FROM review_comments WHERE repository=${sql(repository)} AND ${where} ORDER BY updated_at DESC LIMIT 30;`,
    true,
  ) as Array<Record<string, unknown>>;
  notify(
    ctx,
...
export default function (pi: ExtensionApi): void {
  pi.registerCommand("pr-review-archive", {
    description:
      "Archive and search human GitHub PR review evidence in local SQLite",
    handler: async (args, ctx) => {
      try {
        ...
        if (action === "status") {
          initializeDatabase();
          const rows = sqlite(
            "SELECT (SELECT count(*) FROM review_comments) AS comments, (SELECT count(*) FROM pull_requests) AS prs;",
            true,
          ) as Array<Record<string, unknown>>;
          return notify(
            ctx,
            `Archive: ${DATABASE_PATH}\n${JSON.stringify(rows[0] ?? {})}`,
          );
        }
        notify(
          ctx,
          "Usage: /pr-review-archive sync [--repo owner/repo] [--authors user1,user2] [--limit 100] | show <PR-number> | search <terms> | status",
          "warning",
        );
      } catch (error) {
        notify(
          ctx,
          `PR review archive failed: ${error instanceof Error ? error.message : String(error)}`,
          "error",
        );
      }
    },
