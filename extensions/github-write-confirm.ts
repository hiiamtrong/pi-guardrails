...
function hasUnsafeApiArguments(tokens: string[]): boolean {
  return tokens.some((token, index) => {
    ...
    if (token.startsWith("-X") && token.length > 2)
      return token.slice(2).toUpperCase() !== "GET";
    return (
      token.startsWith("--method=") &&
      token.slice("--method=".length).toUpperCase() !== "GET"
    );
  });
}
...
function isReadOnlyGhCommand(command: string): boolean {
  ...
  const commandIndex = tail.findIndex((token) =>
    READ_ONLY_GH_COMMANDS.has(token.toLowerCase()),
  );
  if (commandIndex === -1) return false;
  const ghCommand = tail[commandIndex].toLowerCase();
  const subcommand = tail
    .slice(commandIndex + 1)
    .find((token) => !token.startsWith("-"))
    ?.toLowerCase();
  if (ghCommand === "api")
    return !hasUnsafeApiArguments(tail.slice(commandIndex + 1));
...
function isGithubCommand(command: string): boolean {
  return (
    splitShellWords(command).some((token) =>
      /(?:^|\/)gh(?:\.exe)?$/i.test(token),
    ) || /(?:api\.)?github\.com/i.test(command)
  );
}
...
function isReadOnlyGithubHttpCommand(command: string): boolean {
  if (
    !/(?:api\.)?github\.com/i.test(command) ||
    !/\b(curl|wget|http)\b/i.test(command)
  )
    return false;
  return !/\s(?:-X|--request)\s*(?:POST|PUT|PATCH|DELETE)|(?:-X|--request)=(?:POST|PUT|PATCH|DELETE)|\s(?:-d|--data|--data-raw|--data-binary|--form)\b/i.test(
    command,
  );
}
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    const command =
      typeof event.input?.command === "string"
        ? redactSecrets(event.input.command)
        : undefined;
