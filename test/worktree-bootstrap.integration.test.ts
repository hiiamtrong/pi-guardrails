  ...
  try {
    ...
    extension({
      registerCommand(
        name: string,
        command: {
          handler: (args: string, ctx: { cwd: string; hasUI?: boolean }) => Promise<void>;
        },
      ) {
    ...
    assert.equal(readFileSync(join(worktree, ".env"), "utf8"), "PRIVATE_VALUE=kept-local\n");
    assert.match(readFileSync(hookPath, "utf8"), /pi-guardrails-worktree-bootstrap/);
    assert.equal(
      readFileSync(`${hookPath}.pi-guardrails-original`, "utf8"),
      originalHook,
    );

    await handler("disable", { cwd: repo, hasUI: true, ui: { notify: () => {} } });
